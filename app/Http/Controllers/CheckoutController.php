...
class CheckoutController extends Controller
{
    ...
    public function confirm_order(Request $request){
         ...
         $checkout_code = substr(md5(microtime()),rand(0,26),5); // code random đơn hàng
         ...
         $order->created_at = now(); // hàm lấy ngày hiện tại
         $order->save();

         if(Session::get('cart')==true){
            foreach(Session::get('cart') as $key => $cart){
                $order_details = new OrderDetails;
                $order_details->order_code = $checkout_code; // so sánh tbl_order vs tbl_order_details
                $order_details->product_id = $cart['product_id']; //$cart lấy ra = session cart
                $order_details->product_name = $cart['product_name'];
                $order_details->product_price = $cart['product_price'];
                $order_details->product_sales_quantity = $cart['product_qty'];
                $order_details->product_coupon =  $data['order_coupon']; // $data bằng trường gửi qua = ajax bên layout
    ...
    //     Session::forget('fee'); // xóa phí vận chuyển
    ...
    public function checkout(Request $request){
        ...
        return view('pages.checkout.show_checkout')->with('category',$cate_product)->with('brand',$brand_product)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider);
    }
